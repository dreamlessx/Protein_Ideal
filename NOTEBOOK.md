...
## Final State (Locked Snapshot 2026-04-27a)

| Stage | Blue | Green |
|---|---|---|
| AlphaFold 2.3.2 (5 replicates) | 257/257 | 257/257 |
| AF built-in AMBER | 257/257 | 257/257 |
| AF unrelaxed | 257/257 | 257/257 |
| Boltz-1 v0.4.1 (5 replicates) | 257/257 | 257/257 |
| Standalone AMBER (AF) | 257/257 | 257/257 |
| Standalone AMBER (Boltz) | 257/257 | 257/257 |
| Standalone AMBER (crystal) | 257/257 | 257/257 |
| Rosetta `.pdb.gz` outputs | 208,170 | 208,170 |
| Rosetta MolProbity rows | 208,170 | 208,170 |

Combined Blue + Green frame: 416,340 / 416,340 Rosetta MolProbity rows. 0 gap cells, 0 missing rows, 0 NaN. 663 exact-duplicate rows + 27 legacy-source rows filtered at ingest. `qc_status = pass`.

Resolved issues:
- 11 prior Boltz-OOM targets (1DE4, 1GXD, 1K5D, 1N2C, 1WDW, 1ZM4, 3BIW, 3EO1, 3L89, 4GXU, 6EY6) handled via FASTA dedup; all 257 BM5.5 targets active.
- 1ACB and 1ATN AMBER-crystal convergence failures: resolved via v5 chain-split preprocessing.
- ACCRE node cn1340 (1,614+ failures): excluded from all SLURM scripts via `#SBATCH --exclude=cn1340`.
- 12 corrupt `.pdb.gz` files identified and refilled (jobs 10317990, 10349662, 10352207).

Companion repos:
- `dreamlessx/Protein_Relax_Pipeline`: canonical Blue + Green merged red_analysis + DB + figures + pptx.
- `dreamlessx/Protein_Data_Analysis`: 20-target validation subset (MolProbity + PoseBusters).

Per-target arithmetic: 27 input structures x 6 protocols x 5 replicates = 810 Rosetta runs/target. 7 source buckets (af_relaxed, af_unrelaxed, amber_af, amber_boltz, amber_crystal, boltz, crystal). 6 protocols (cartesian_beta, cartesian_ref15, dualspace_beta, dualspace_ref15, normal_beta, normal_ref15). 16 AMBER runs/complex (5 AF built-in + 5 AF standalone + 5 Boltz standalone + 1 crystal standalone).
...
- Same issue Pipeline reported on 1AHW and 1ATN
...
### Input Verification ( Directive #4/#5)
...
's DNA/RNA exclusion policy (protein-only predictions), audited
...
- Deleted temp verification scripts from external coordination/ (fasta_comparison.sh, etc.)
...
### external coordination Repo Restructured

external coordination was restructured by operations:
...
- Hierarchy formalized: dreamlessx → operations → Workers (Blue, Green, Teal)
...
Per operations directive, created `INSTRUCTIONS.md` in Protein_Ideal repo root with:
